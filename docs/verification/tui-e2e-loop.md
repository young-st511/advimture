...
- `screen_final.txt`: terminal size가 있는 scenario에서는 terminal cell grid 기준 final viewport evidence. fallback으로 마지막 `ADVIMTURE |` 또는 playable error frame 중심의 cleaned final screen을 사용한다.
