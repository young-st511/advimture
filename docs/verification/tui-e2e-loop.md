...
- `summary.json`: pass/fail, error, exit code, HOME, key trace, screen byte 수, progress file/app state 존재 여부, raw evidence 여부, artifact 저장 여부
- `raw.log`: raw ANSI stream
- `screen.txt`: cleaned terminal text stream
- `screen_timeline.txt`: cleaned terminal text를 UI QA용 누적 흐름 evidence로 명시 저장한 파일
- `screen_final.txt`: terminal size가 있는 scenario에서는 terminal cell grid 기준 final app viewport evidence. fallback으로 마지막 `ADVIMTURE |` 또는 playable error frame 중심의 cleaned final screen을 사용한다. double-width 문자 continuation cell은 사람이 읽는 문자열에 공백처럼 끼어들지 않아야 한다.
...
긴 incident route와 command-choice applied route는 사람이 마지막 화면만 빠르게 읽는 `screen_final.txt`와, 실패/성공 흐름을 따라가는 `screen_timeline.txt`를 함께 남긴다. 이 evidence bundle은 UI/UX RedTeam, Agent playtest, 회귀 분석의 기본 관찰 단위로 사용한다.
