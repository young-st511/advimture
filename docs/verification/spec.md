...
- 긴 incident full route와 command-choice applied route는 route 전체 흐름과 마지막 화면을 재검토할 수 있도록 screen timeline/final screen evidence를 함께 남긴다.
- `make e2e-playable`은 첫 문제 smoke, constraint/coaching UX, playlist next, progress resume, `:q!` command-line, `:s/api/web/` substitute, first-cut tutorial flow, operator/yank/text-object/open-line/repeat/search/quote text object full playpack, incident 001~004, command-choice scope, incident 006/007 run 경로를 모두 검증한다.
