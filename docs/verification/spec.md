...
- `make e2e-playable`은 첫 문제 smoke, playlist next, progress resume, `:q!` command-line, `:s/api/web/` substitute, first-cut tutorial flow, operator/yank/text-object/open-line/repeat/search full playpack 경로를 모두 검증한다.
