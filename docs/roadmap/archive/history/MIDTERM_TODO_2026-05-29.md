...
| 7 | SUCCESS-MODAL-001 | completed | success modal heading과 record density 정리 |
| 8 | UX-BACKLOG-001 | completed | 다음 content breadth로 넘어가기 전 playtest backlog 정리 |
| 9 | CONTENT-BREADTH-001 | completed | 기존 엔진만 사용해 command-choice/applied incident 폭 확장 |
...
| SUCCESS-MODAL-001 | success modal 중복 리스크 확인 | completed: 성공 modal heading/density polish | completed: renderer/playable tests, focused E2E, `make e2e-playable` | 다음 행동 line이 잘리지 않게 유지 |
| UX-BACKLOG-001 | polish pass 완료 | completed: 다음 중기 플랜 후보와 backlog | completed: docs review, `git diff --check` | 새 engine 기능을 무계획으로 열지 않음 |
| CONTENT-BREADTH-001 | backlog 우선순위 확정 | completed: `choice-002-repeat-or-substitute` playable beat + E2E | completed: content replay, focused E2E, `go test ./...`, `make e2e-playable` | 기존 엔진으로 가능한 폭만 확장 |
...
## Foundation Playtest / UX Polish 완료 판단

2026-05-26 기준 이 중기 플랜은 completed다. HUD 밀도, hint/retry/quit affordance, command-choice 판단 copy, success modal density를 보강했고, UX backlog에서 content breadth를 막는 P0 blocker가 없음을 확인했다. 이어서 `choice-002-repeat-or-substitute`를 playable로 추가해 command-choice incident를 2-beat로 확장했다.

## 다음 중기 플랜: Inline Target Motions

> 목표는 한 줄 안의 delimiter/quote/comma를 빠르게 잡는 `f/t` 엔진을 열고, operator 조합과 tutorial playpack까지 연결하는 것이다.

| 순서 | ID | 상태 | 목표 |
|------|----|------|------|
| 1 | CHAR-FIND-GAP-001 | completed | forward same-line `f/t`와 `df/dt/cf/ct` 첫 scope 고정 |
| 2 | VIM-030 | completed | char-find pending state, normal motion, delete/change operator 결합 구현 |
| 3 | PLAYPACK-012 | completed | 6문항 char-find-line tutorial content/E2E 구현 |
| 4 | CHAR-FIND-APPLIED-001 | completed | command-choice 또는 incident에 inline target motion 적용 후보 설계 |
| 5 | CHOICE-PLAY-002 | completed | incident-005에 `ct,` inline target range choice beat 추가 |

## Inline Target Motions 출구 조건

| ID | 입구 조건 | 필수 산출물 | 검증 | 품질 저하 방지 |
|----|----------|-------------|------|---------------|
| CHAR-FIND-GAP-001 | Foundation polish와 command-choice 확장 완료 | completed: `char-find-line` command cluster approval packet, VIM-030/PLAYPACK-012 분리 계획 | completed: `git diff --check` | `F/T`, `;`, `,`, count, visual, yank 결합을 첫 구현에 넣지 않음 |
| VIM-030 | CHAR-FIND-GAP-001 완료 | completed: `f/t`, `df/dt/cf/ct` engine/runtime/tuiadapter tests | completed: vimengine/tuiadapter/runtime tests, `go test ./...` | engine과 content/E2E를 섞지 않음 |
| PLAYPACK-012 | VIM-030 완료 | completed: char-find-line tutorial YAML, scenario, playlist, focused E2E | completed: content replay, full playlist E2E, `make e2e-playable` | 8문항 이하, required key와 forbidden 우회를 고정 |
| CHAR-FIND-APPLIED-001 | PLAYPACK-012 완료 | completed: `choice-005-inline-target-range` 추천, `incident-006-inline-target-repair` 후속 후보 | completed: docs review | 새 engine 기능을 추가하지 않고 적용만 다룸 |
| CHOICE-PLAY-002 | CHAR-FIND-APPLIED-001 완료 | completed: command-choice third beat, scenario, focused E2E | completed: content replay, focused E2E, full regression | `ct,` 선택 판단만 다루고 search+inline 복합 incident는 보류 |

2026-05-26 기준 이 중기 플랜은 completed다. `char-find-line`은 scope planning, engine, tutorial, command-choice applied beat까지 닫혔다. 다음 후보는 `/target` 검색 후 `ct,`로 comma 앞 값만 교체하는 `incident-006-inline-target-repair` 또는 아직 playable로 승격하지 않은 reuse-choice drill이다.

## 다음 중기 플랜: Applied Mastery Runs

> 목표는 새 Vim engine을 바로 늘리지 않고, 이미 배운 command를 실제 incident와 command-choice 판단 문제에서 조합하게 만드는 것이다.

| 순서 | ID | 상태 | 목표 |
|------|----|------|------|
| 1 | PLAN-REFRESH-008 | completed | Applied mastery 중기 플랜 문서화 |
| 2 | INCIDENT-006 | completed | `/target` + `ct,` inline target repair incident 구현 |
| 3 | REUSE-CHOICE-001 | completed | `yi"`, `yy/p`, `.`, retype 사이의 재사용 판단 드릴 설계 |
| 4 | CHOICE-PLAY-003 | completed | quote value reuse-choice playable 구현 |
| 5 | INCIDENT-007 | completed | search, text object, substitute, visual-line, char-find 조합 mixed run 확장 |
| 6 | PLAYTEST-REVIEW-002 | completed | RedTeam/UX/evidence 점검과 다음 후보 정리 |

## Applied Mastery Runs 출구 조건

| ID | 입구 조건 | 필수 산출물 | 검증 | 품질 저하 방지 |
|----|----------|-------------|------|---------------|
| PLAN-REFRESH-008 | Inline Target Motions 완료 | completed: 중기 플랜, INCIDENT-006 active ExecPlan | completed: `git diff --check` | 새 기능 구현 없이 roadmap만 정리 |
| INCIDENT-006 | PLAN-REFRESH-008 완료 | completed: applied incident YAML, scenario, playlist, focused E2E | completed: content replay, focused E2E, full regression | 새 engine 없이 `/`, `ct,` 조합만 사용 |
| REUSE-CHOICE-001 | INCIDENT-006 완료 | completed: `choice-006-quote-value-reuse` 선택, linewise/dot 후보 보류 | completed: docs review | 구현 전 범위와 우회 방지 기준을 먼저 고정 |
| CHOICE-PLAY-003 | REUSE-CHOICE-001 완료 | completed: reuse-choice playable beat, scenario, E2E | completed: content replay, focused E2E, full regression | 직접 재입력 우회를 constraints로 막음 |
| INCIDENT-007 | CHOICE-PLAY-003 완료 | completed: mixed recovery run | completed: focused E2E, `make e2e-playable` | 너무 많은 command를 한 beat에 몰아넣지 않음 |
| PLAYTEST-REVIEW-002 | INCIDENT-007 완료 | completed: review report, next candidate | completed: full test/evidence review | 부족한 E2E 발견 시 보강을 우선 |

2026-05-28 기준 이 중기 플랜은 completed다. Applied mastery는 `incident-006`, reuse-choice, `incident-007`까지 닫혔다. 다음 권장 후보는 새 content나 engine보다 `E2E-EVIDENCE-008`로 긴 incident의 final/timeline evidence를 보강하는 것이다.

## 다음 중기 플랜: Long Incident Evidence Hardening

> 목표는 새 Vim 기능이나 content를 늘리기 전에, 긴 incident route를 Agent와 사람이 같은 evidence로 재검토할 수 있게 만드는 것이다.

| 순서 | ID | 상태 | 목표 |
|------|----|------|------|
| 1 | E2E-EVIDENCE-008 | completed | 긴 incident와 command-choice applied route의 final/timeline evidence bundle 고정 |
| 2 | PLAYTEST-REVIEW-003 | completed | 보강된 evidence로 다음 content/engine 후보를 재점검 |

## Long Incident Evidence Hardening 출구 조건

| ID | 입구 조건 | 필수 산출물 | 검증 | 품질 저하 방지 |
|----|----------|-------------|------|---------------|
| E2E-EVIDENCE-008 | Applied Mastery Runs 완료 | incident full route와 command-choice scope route의 `screen_timeline.txt`, `screen_final.txt` evidence | runner test, focused E2E, full playable E2E, 전체 Go test | runner 기능/TUI layout/progress schema를 이번 slice에서 바꾸지 않음 |
| PLAYTEST-REVIEW-003 | long route evidence 보강 완료 | evidence 기반 다음 후보 추천 | docs review, 필요 시 focused E2E 재확인 | evidence 없이 새 engine/content를 열지 않음 |

2026-05-29 기준 이 중기 플랜은 completed다. 긴 incident와 command-choice applied route는 final/timeline evidence를 남기며, 대표 route와 full playable E2E를 통과했다. 다음 권장 후보는 `PLAN-REFRESH-009`로 Foundation exit review와 다음 중기 플랜을 먼저 정리한 뒤, 새 engine 후보인 quote-pair hardening 또는 platform review를 고르는 것이다.
