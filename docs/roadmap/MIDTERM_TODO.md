...
| 3 | UI-CONTRACT-001 | pending | header/briefing/console/status/action 영역 contract 고정 |
| 4 | UI-RENDER-001 | pending | `internal/playable`에서 순수 renderer 패키지 분리 |
| 5 | UI-HIERARCHY-001 | pending | 현재 exercise 목표를 상위로 올리고 review/daily를 보조 정보로 낮춤 |
| 6 | UI-MODE-001 | pending | tutorial action panel과 incident action panel 언어 분리 |
| 7 | UI-EVIDENCE-001 | pending | final frame 또는 frame timeline evidence 보강 |
...
| UI-CONTRACT-001 | UX-QA-001 완료 | 화면 영역 contract와 첫 렌더링 slice 수용 기준 | docs review, `git diff --check` | 코드 구현 전 정보 위계 고정 |
| UI-RENDER-001 | 영역 contract 완료 | 순수 renderer 패키지와 기존 behavior 보존 | playable/renderer tests, smoke E2E | Update/game state 변경 금지 |
| UI-HIERARCHY-001 | renderer 분리 완료 | 현재 목표 우선 화면 구조 | model tests, focused E2E evidence | review/daily는 보조 정보 유지 |
| UI-MODE-001 | hierarchy 완료 | tutorial/incident action panel 분리 | playable tests, incident E2E | incident에서 정답 키 즉시 노출 과다 방지 |
| UI-EVIDENCE-001 | UI 변경 반복 부담 확인 | final frame/timeline evidence | e2e-runner tests | timing flake를 늘리지 않음 |
