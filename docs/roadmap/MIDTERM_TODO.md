...
| 2 | FTUE-001 | active | 첫 5분 canonical route 정의와 evidence 점검 |
| 3 | UI-PLAYTEST-001 | planned | Mission HUD + floating modal 사용성 evidence 기반 polish |
| 4 | CHOICE-PLAY-001 | planned | command-choice drill을 playable content/E2E로 승격 |
| 5 | DAILY-ROUTE-001 | planned | 저장 포맷 변경 없는 daily/review 반복 동기 강화 |
| 6 | PROGRESS-V2-DECISION-001 | planned | v1 한계 확인 후 progress schema v2 승인/보류 결정 |
...
| FTUE-001 | current playable route 존재 | 첫 5분 route, 학습 감정 곡선, representative evidence | focused E2E 또는 `make e2e-playable` | 새 command/content schema 추가 금지 |
| UI-PLAYTEST-001 | FTUE route 고정 | 주요 running/failed/succeeded screen evidence와 polish backlog | renderer/playable tests, UI-focused E2E | split cockpit으로 바로 확장하지 않음 |
