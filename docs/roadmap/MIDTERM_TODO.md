...
| 4 | PLAYPACK-011 | pending | 3문항 이하 linewise visual tutorial content/E2E 구현 |
| 5 | INCIDENT-RUN-004 | pending | linewise visual을 적용하는 config block 복구 incident 구현 |
| 6 | COMMAND-CHOICE-001 | pending | 배운 command 중 적절한 도구를 고르는 mixed drill 설계 |
| 7 | PLATFORM-REVIEW-002 | pending | 저장 포맷 변경 없는 review/daily 동기 강화 |
...
| PLAYPACK-011 | VIM-029 완료 | 3문항 이하 tutorial, full E2E | content replay, focused E2E, `make e2e-playable` | scenario보다 command 차이를 먼저 가르침 |
| INCIDENT-RUN-004 | linewise tutorial 완료 | config block 복구 incident | content replay, incident E2E | 새 command 소개 금지, 적용만 수행 |
| COMMAND-CHOICE-001 | incident 004 완료 | command choice drill 설계 | docs review | 정답 암기보다 도구 선택 이유를 강화 |
| PLATFORM-REVIEW-002 | review queue baseline 존재 | 저장 변경 없는 review/daily UX 개선 | playable tests, focused E2E | progress v2는 별도 승인 전 금지 |
