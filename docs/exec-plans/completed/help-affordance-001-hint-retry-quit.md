...
- [x] existing playable tests와 E2E coverage 확인
- [x] 누락된 affordance route 식별

## Step 2: Tests and UX Copy

- [x] tutorial hint E2E 또는 app_state assertion 추가/보강
- [x] incident hint E2E 추가
- [x] retry/quit route 보강
- [x] 필요한 문구 조정
...
- 첫 입력 전에도 `?` hint가 무반응이 되지 않도록 runtime hint contract를 보강했다.
- `playable_incident_hint_affordance` E2E를 추가해 incident running 상태의 `?`와 `q` affordance를 검증한다.
- command/search/insert/visual mode는 mode-specific 안내만 표시하고 일반 `?: hint q: quit` 안내를 섞지 않는 테스트를 유지/보강했다.
