...
- [x] 기존 docs candidate를 playable 범위로 구체화
- [x] command cluster/status 결정
- [x] playlist 위치와 category 결정
...
- command-choice는 새 command cluster가 아니라 적용 레이어로 둔다.
- 첫 playable은 `visual-line-basic` command cluster에 얹은 `command-choice-scope-001` 한 문항으로 시작한다.
- playlist는 tutorial이 아니라 `incident` category의 order 5, `incident-005-command-choice`로 둔다.
- 의도 선택은 `constraints.required_keys: ["V", "d"]`와 `max_inputs: 3`, quote/word/manual 우회 금지로 고정한다.
