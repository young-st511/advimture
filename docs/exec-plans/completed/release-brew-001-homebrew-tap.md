...
- 2026-06-17: 사용자가 `0.2.0 배포`를 명시했다. `scripts/release.sh minor`로 `VERSION=0.2.0`, release commit `cd95c8d`, tag `v0.2.0`을 만들고 `main`/tag를 push했다. GitHub Actions Release run `27656189915`가 성공했고 Homebrew tap formula는 `v0.2.0` source archive와 sha256으로 갱신됐다.
