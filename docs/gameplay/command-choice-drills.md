...
## Line Reuse Choice Decision

`LINE-REUSE-APPLIED-001`의 적용 후보는 `choice-007-line-reuse`로 한다.

이유:

- `V` + `y` + `p`는 이미 tutorial과 incident에서 engine/replay/E2E 경로가 검증됐다.
- command-choice에서는 새 command 소개가 아니라 "값을 다시 칠지, 검증된 줄 전체를 재사용할지"를 판단하게 만들 수 있다.
