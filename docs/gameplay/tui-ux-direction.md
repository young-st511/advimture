...
FocusPanel
  tutorial: TRAINING BRIEF
  incident: OPERATOR JUDGMENT
  failed: RECOVERY REQUIRED
  succeeded: STEP SEALED
  mode-specific: COMMAND/SEARCH/INSERT/VISUAL CHANNEL
...
- FocusPanel: `TRAINING BRIEF`로 `훈련 키`, `힌트`, `재시도`, `다음`을 console 위에 명시한다.

Tutorial에서는 `재점검 대상`, `오늘의 복구 루트`가 현재 목표보다 앞에 오지 않는다. 필요하면 FocusPanel 또는 side rail의 보조 정보로 낮춘다.
...
- FocusPanel: `OPERATOR JUDGMENT`로 `판단`, `힌트`, `재시도`, `다음 조치`를 console 위에 표시한다.
...
- true 중앙 overlay는 `tea.WindowSizeMsg`로 terminal width/height를 모델에 저장하는 별도 slice에서 다룬다.
