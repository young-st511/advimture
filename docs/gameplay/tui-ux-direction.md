...
Foundation UI pass 이후 화면 정보 계층은 Mission HUD, Runbook Console, floating modal, status line으로 정리됐다. 다만 출시용 게임 UI로 보려면 color/emphasis, wide layout, command memory 같은 polish가 아직 남아 있다.
...
- Foundation polish 이후 content breadth와 quote hardening을 막는 P0 UX 리스크는 없다. 남은 UI 작업은 `docs/roadmap/UX_BACKLOG_001.md`의 backlog로 관리한다.
...
- 과거 `Retry`/`Next runbook` 같은 action line은 `UI-CONSOLE-POLISH-001`에서 한국어 screen label과 machine-readable action id로 분리했다. 이후 UI copy 변경은 `actions.id`를 유지한 채 label만 조정한다.
