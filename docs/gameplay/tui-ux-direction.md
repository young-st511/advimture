...
- 다음 polish는 설명형 텍스트를 더 늘리는 대신, 1줄짜리 ASCII signal rail과 input echo로 "입력하면 장면이 반응한다"는 감각을 먼저 만든다.
...
Mission HUD
  ...
  tutorial tools or incident judgment cue
  SIGNAL rail: compact ASCII scene feedback and last input echo
  ACTIONS bar: compact interactive controls
...
- Mission HUD cue: `TOOLS`로 `훈련 키`와 `기억할 명령`을 현재 목표 바로 아래에 압축한다.
- Mission HUD cue는 `기억할 명령: ...`을 짧게 보여줘, 콘텐츠가 늘어나도 방금 배우는 Vim command를 놓치지 않게 한다.

Tutorial에서는 `재점검 대상`, `오늘의 복구 루트`가 현재 목표보다 앞에 오지 않는다. 80-column running 화면에서는 기본적으로 숨기고, success/debrief 또는 후속 wide rail로 낮춘다.

Incident running 화면에서도 review/daily는 현재 목표와 console 접근을 늦추지 않는다. 후속 wide rail이 열리기 전까지는 success/debrief에서 더 강하게 다룬다.
...
- Mission HUD cue: `JUDGMENT`로 판단 목표를 현재 목표와 함께 표시한다.
...
19. **UI-ADVENTURE-BEAT-001**: running HUD에 1줄 `SIGNAL` rail, lightweight animation tick, 마지막 입력 echo를 추가해 console 조작이 화면에서 짧게 반응하게 했다.

## 후속 UI 방향

- Foundation polish 이후 modal/action hierarchy P0/P1 리스크는 닫혔고, `UI-ADVENTURE-BEAT-001`에서 1줄 signal rail, input echo, lightweight animation을 적용했다.
...
Foundation UI는 QA 기반, renderer 분리, Mission HUD, floating modal, final/timeline evidence, running signal rail까지 완료됐다. 다음 UI 작업은 `docs/roadmap/UX_BACKLOG_001.md`의 P1/P2 backlog를 기준으로 열고, `PLAN-REFRESH-009`에서 content/platform 우선순위와 함께 다시 판단한다.
