...
- `다음 단계`, `다음 튜토리얼`, `다음 runbook`, `다음 출격`, `출격 완료`: modal footer의 primary action
...
- Foundation polish 이후 진행 불가 P0 UX 리스크는 없지만, modal/action hierarchy는 출시 가능한 수준 기준의 P1 release-blocker로 active 처리한다.
